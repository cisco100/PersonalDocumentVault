...
def upload_file(request, folder_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Read file data
            UserActivity.objects.create(user=request.user, activity_type='upload')
            file_data = request.FILES['file'].read()
...
def share_file(request, file_id):
    ...
    UserActivity.objects.create(user=request.user, activity_type='share')
...
def download_file(request, file_id):
    ...
    UserActivity.objects.create(user=request.user, activity_type='download')
